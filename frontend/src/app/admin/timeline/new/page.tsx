...
export default function NewTimelinePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!token) {
        alert("Please login to create a timeline entry");
        router.push("/login");
        return;
      }

      await createTimelineEntry(token, formData);
      router.push("/admin/timeline");
    } catch (error: any) {
      console.error("Error creating timeline entry:", error);
      alert(error.message || "An error occurred while creating the timeline entry");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-4xl font-bold text-[#1a1a1a] mb-2">New Milestone</h1>
          <p className="text-[#4a4a4a]">Add a new milestone to your journey timeline</p>
        </div>
      </div>
